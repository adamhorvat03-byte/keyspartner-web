...
exports.handler = async (event, context) => {
  ...
  // GET health-check pre overenie stavu
  ...
  // 1. Zalogovanie prichádzajúceho payloadu z Realsoftu
  ...
  try {
    ...
    try {
      ...
      console.warn("Payload nie je platný JSON, pokus o uloženie surového tela.");
    }
    ...
    // 2. Ak ide o vymazanie alebo deaktiváciu
    const isDelete =
      action === "delete" ||
      action === "deactivate" ||
      raw.status === "deleted" ||
    ...
    if (isDelete) {
      ...
      if (store) {
        try {
          ...
          console.warn("[Netlify Blobs] Chyba pri mazaní z blobu:", delErr.message);
        }
      }
    } else {
      // 3. Normalizácia a uloženie inzerátu (Upsert)
      ...
      if (store) {
        ...
        console.log(`[Netlify Blobs] Inzerát ${externalId} (${propertyItem.title}) úspešne zapísaný do Netlify Blobs.`);
      } else {
        console.log(`[Netlify Blobs Mock] Inzerát ${externalId} spracovaný bez aktívneho úložiska.`);
      }
    }
  } catch (err) {
    console.error("[Realsoft Webhook] Chyba pri spracovaní inzerátu:", err);
  }

  // 4. Oficiálna odpoveď pre Realsoft
  return {
    statusCode: 200,
    headers,
    body: JSON.stringify({
